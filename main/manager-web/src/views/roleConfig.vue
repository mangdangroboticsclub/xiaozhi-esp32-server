            ...
            <div class="config-header">
              ...
              <div class="header-actions">
                <div class="hint-text">
                  ...
                  <span>After saving configuration, restart the device to enable new configuraiton </span>
                </div>
                ...
                <div class="form-grid">
                  <div class="form-column">
                    ...
                    <el-form-item label="Language code：" style="display: none;">
                      <el-input v-model="form.langCode" placeholder="Please enter language code, e.g., zh_CN" maxlength="10" show-word-limit
                        class="form-input" />
                    </el-form-item>
                    <el-form-item label="Interaction Language：" style="display: none;">
                      <el-input v-model="form.language" placeholder="Please enter interaction language, e.g., Chinese" maxlength="10" show-word-limit
                        class="form-input" />
                    </el-form-item>
                  </div>
                  <div class="form-column">
                      ...
                      <div class="model-select-wrapper">
                        ...
                        <div v-if="model.type === 'Memory' && form.model.memModelId !== 'Memory_nomem'"
                          ...
                          <el-radio-group v-model="form.chatHistoryConf" @change="updateChatHistoryConf">
                            <el-radio-button :label="1">Upload Text</el-radio-button>
                            <el-radio-button :label="2">Upload Text and Speech</el-radio-button>
                          </el-radio-group>
                        </div>
                      </div>
                    </el-form-item>
                    <el-form-item label="Role Voice">
...
export default {
  ...
  data() {
    return {
      ...
      models: [
        { label: 'Voice Activity Detection', key: 'vadModelId', type: 'VAD' },
        { label: 'Automatic Speech Recognition', key: 'asrModelId', type: 'ASR' },
        { label: 'LLM', key: 'llmModelId', type: 'LLM' },
        { label: 'VLLM', key: 'vllmModelId', type: 'VLLM' },
        { label: 'Intent Recognition', key: 'intentModelId', type: 'Intent' },
        { label: 'Memory', key: 'memModelId', type: 'Memory' },
        { label: 'Text to Speech', key: 'ttsModelId', type: 'TTS' },
      ],
      modelOptions: {},
      templates: [],
  ...
  methods: {
    ...
    saveConfig() {
      ...
      Api.agent.updateAgentConfig(this.$route.query.agentId, configData, ({ data }) => {
        if (data.code === 0) {
          ...
          this.$message.error({
            message: data.msg || 'Failed to save config',
            showClose: true
          });
        }
      });
    ...
    resetConfig() {
      this.$confirm('confirm to reset config?', 'NOTE', {
        ...
        this.$message.success({
          message: 'Configuration Resetted',
          showClose: true
        })
      }).catch(() => {
      });
    ...
    fetchAgentConfig(agentId) {
      Api.agent.getDeviceConfig(agentId, ({ data }) => {
        if (data.code === 0) {
          ...
          this.$message.error(data.msg || 'failed to get config');
        }
      });
    },
    fetchModelOptions() {
      this.models.forEach(model => {
        Api.model.getModelNames(model.type, '', ({ data }) => {
          if (data.code === 0) {
            ...
            this.$message.error(data.msg || 'Failed to get config list');
          }
        });
      });
    },
      ...
      return new Promise((resolve, reject) => {
        Api.model.getPluginFunctionList(null, ({ data }) => {
          if (data.code === 0) {
            ...
            this.$message.error(data.msg || 'Falied to get plugin list');
            reject();
          }
        });
      });
