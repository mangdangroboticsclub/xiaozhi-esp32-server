  ...
  <el-dialog :visible.sync="dialogVisible" :close-on-click-modal="false" width="57%" center custom-class="custom-dialog" :show-close="false"
    ...
    <div style="margin: 0 18px; text-align: left; padding: 10px; border-radius: 10px;">
      ...
      <el-form :model="form" ref="form" label-width="100px" label-position="left" class="custom-form">
        ...
        <div style="display: flex; gap: 20px; margin-bottom: 0;">
          <el-form-item label="Supplier" prop="supplier" style="flex: 1;">
          ...
          <el-form-item label="Sort Number" prop="sort" style="flex: 1;">
            <el-input v-model.number="form.sort" type="number" placeholder="please enter sort number" class="custom-input-bg"></el-input>
          </el-form-item>
        </div>
        ...
        <el-form-item label="Remark" prop="remark" class="prop-remark">
          <el-input v-model="form.remark" type="textarea" :rows="3" placeholder="please enter model remark" :autosize="{ minRows: 3, maxRows: 5 }"
            class="custom-input-bg"></el-input>
        </el-form-item>
      </el-form>

      <div style="font-size: 20px; font-weight: bold; color: #3d4566; margin-bottom: 15px;">Call Message</div>
      <div style="height: 2px; background: #e9e9e9; margin-bottom: 22px;"></div>
          ...
          <div :key="rowIndex" style="display: flex; gap: 20px; margin-bottom: 0;">
            <el-form-item v-for="field in row" :key="field.prop" :label="field.label" :prop="field.prop"
              ...
              <template v-if="field.type === 'json-textarea'">
                <el-input v-model="fieldJsonMap[field.prop]" type="textarea" :rows="3" placeholder="please enter variable in JSON format(e.g., :{'key':'value'})"
  ...
  methods: {
    ...
    validateJson(value) {
      try {
        ...
        this.$message.error({
          message: 'must enter dictionary format（e.g. {"key":"value"}），use original data to save',
          showClose: true
        });
        return null;
      } catch (e) {
        this.$message.error({
          message: 'Wrong JSON format(e.g. {"key":"value"}, use original data to save',
          showClose: true
        });
        return null;
      }
