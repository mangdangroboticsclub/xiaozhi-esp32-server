  ...
  <el-dialog :visible="visible" @close="handleClose" width="24%" center>
    ...
    <div style="margin: 22px 15px;">
      <div style="font-weight: 400;font-size: 14px;text-align: left;color: #3d4566;">
        ...
        <span style="font-size: 11px"> Device Code：</span>
      </div>
      <div class="input-46" style="margin-top: 12px;">
        <el-input placeholder="Please enter the 6-digit device code.." v-model="deviceCode" @keyup.enter.native="confirm" />
      </div>
    </div>
  ...
  methods: {
    confirm() {
      if (!/^\d{6}$/.test(this.deviceCode)) {
        this.$message.error('Please enter the 6-digit device code');
        return;
      }
      this.loading = true;
      Api.device.bindDevice(
        ...
        this.deviceCode, ({ data }) => {
          ...
          if (data.code === 0) {
            ...
            this.$message.error({
              message: data.msg || 'Device binding failed',
              showClose: true
            });
          }
        }
